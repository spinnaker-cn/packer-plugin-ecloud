...
type Artifact struct {
	EcloudImages   map[string]string
...
func (a *Artifact) Id() string {
	parts := make([]string, 0, len(a.EcloudImages))
	for region, imageId := range a.EcloudImages {
...
func (a *Artifact) String() string {
	...
	return fmt.Sprintf("Ecloud images(%s) were created.\n\n", strings.Join(parts, "\n"))
}
...
func (a *Artifact) Destroy() error {
	...
	//
	//for region, imageId := range a.EcloudImages {
	//	log.Printf("Delete ecloud image ID(%s) from region(%s)", imageId, region)
	//
